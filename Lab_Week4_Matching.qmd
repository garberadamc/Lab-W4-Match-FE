...
4. Estimate regression models for each outcome using the matched sample
5. Compare to alternative matching method (Propensity score matching)
...
```{r}
data_clean <- read_csv(here("data", "Ahmadia_2015_DataClean.csv"), show_col_types = FALSE)
```
...
Visuallize balance on the covariates using `plot()` with `type = "density"`
```{r}
...
```{r}
data_fe <- read_csv(here("data", "Dudney2021_study_data.csv"), show_col_types = FALSE) %>% 
